...
(function ($) {
  var $notice_container = $(".vireo-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.vireo_admin) {
    return;
  }

  var _window$vireo_admin = window.vireo_admin,
      builderStatusData = _window$vireo_admin.builderStatusData,
      getStartedData = _window$vireo_admin.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.vireo-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".vireo-admin-big-notice").addClass("processing");
      $(".vireo-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".vireo-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.vireo-admin-big-notice");
    });
  }
  ...
  var $notice_root = $notice_container.closest(".vireo-admin-big-notice");
  var $custom_close_button = $notice_root.find(".vireo-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".vireo-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.vireo_admin.showOverlay = showOverlay;
})(jQuery);
})();
