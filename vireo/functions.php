...
function vireo_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'vireo_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function vireo_theme() {
	return Theme::getInstance();
}

function vireo_assets() {
	return vireo_theme()->getAssetsManager();
}
...
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	vireo_assets()
		...
		->registerStylesheet( 'vireo-theme', '/theme/theme.css' );
	vireo_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		vireo_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'vireo_print_color_scheme', 0 );
} else {
	vireo_assets()
		->registerStyle( 'vireo-theme', vireo_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function vireo_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'vireo_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'vireo_after_activation_redirect_url' );
function vireo_after_activation_redirect_url( $url ) {
...
function vireo_get_builder_plugin_slug() {
...
function vireo_theme_plugins( $plugins ) {

	$plugin_slug = vireo_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'vireo' ) : __( 'Kubio', 'vireo' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'vireo_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === vireo_get_builder_plugin_slug() ) {
                ...
                <div class="kubio-admin-row get-started-imported kubio-notice notice notice-success">
                    <div>
                        <p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'vireo' ),
									vireo_theme()->getName()
								)
							);
							?>
                        ...
                        <p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'vireo' ),
									vireo_theme()->getName()
								)
							);
							?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'vireo' ) ); ?>
                        </a>
...
function vireo_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'vireo_try_online_url', 10 );

function vireo_render_header_style() {

	?>
    <base target="_top">
	<?php

	vireo_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'vireo_render_header_style', 100 );
