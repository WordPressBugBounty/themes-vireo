(function ($) {
  ...
  function getSiteLeadsBackendData(path, defaultValue) {
    return _.get(top.vireothemeSiteLeadsData, path, defaultValue);
  }
  ...
  const setIsLoadingText = text => {
    const infoNoticeNode = document.querySelector('.vireotheme-siteleads-integration-button__notice--info');
    if (!infoNoticeNode) {
      return;
    }
  ...
  const setIsLoading = newValue => {
    const buttonNode = document.querySelector('.vireotheme-siteleads-integration-button');
    if (!buttonNode) {
      return;
    }
    if (newValue) {
      buttonNode.classList.add('vireotheme-loading');
    } else {
      buttonNode.classList.remove('vireotheme-loading');
      hideInfoNotice();
    }
  };
  ...
  function onUpdateButtonTextAndListeners() {
    buttonNode = document.querySelector('.vireotheme-siteleads-integration-button');
    if (!buttonNode) {
      return;
    }
  ...
  function onUpdateNoticeText() {
    let noticeNodeContainer = document.querySelector('.vireotheme-siteleads-integration-button__container');
    if (!noticeNodeContainer) {
      return;
    }
    let titleNode = noticeNodeContainer.querySelector('.vireotheme-siteleads-integration-button__title');
    ...
    let descriptionNode1 = noticeNodeContainer.querySelector('.vireotheme-siteleads-integration-button__description_1');
    descriptionNode1.innerHTML = getTranslatedText('notice_description_1');
    let descriptionNode2 = noticeNodeContainer.querySelector('.vireotheme-siteleads-integration-button__description_2');
    descriptionNode2.innerHTML = getTranslatedText('notice_description_2');
    let descriptionNode3 = noticeNodeContainer.querySelector('.vireotheme-siteleads-integration-button__description_3');
    descriptionNode3.innerHTML = getTranslatedText('notice_description_3');
  }
  function unUpdateNoticeErrorText() {
    hideInfoNotice();
    let noticeErrorNode = document.querySelector('.vireotheme-siteleads-integration-button__notice--error');
    if (!noticeErrorNode) {
      return;
    }
  ...
  //Siteleads init
  function initApp() {
    let container = document.querySelector('.vireotheme-siteleads-integration-button__container');
    if (!container) {
      return;
    }
